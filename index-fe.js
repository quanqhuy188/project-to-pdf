...
const { FRONTEND_PROJECT_PATH, PDF_OUTPUT_FE_FOLDER } = require("./constants"); // Bạn có thể đổi sang backend nếu muốn

// Kiểm tra định dạng file mà bạn muốn chuyển đổi
const validExtensions = [".html", ".js", ".jsx", ".css"];

// Tạo thư mục PDF nếu chưa tồn tại
if (!fs.existsSync(PDF_OUTPUT_FE_FOLDER)) {
  fs.mkdirSync(PDF_OUTPUT_FE_FOLDER, { recursive: true }); // Đảm bảo tạo tất cả các thư mục cần thiết
}
...
const convertFileToPdf = async (filePath, relativePath) => {
  ...
  const outputDir = path.join(PDF_OUTPUT_FE_FOLDER, path.dirname(relativePath));
...
const main = async () => {
  const files = getAllFiles(FRONTEND_PROJECT_PATH, [], FRONTEND_PROJECT_PATH); // Duyệt tất cả các file trong thư mục
